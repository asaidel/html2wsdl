<wsdl:definitions name="OSB_Cliente_ClaveCrear"
    ...
    <wsdl:types>
		<xs:schema>
			...
			<xs:import schemaLocation="../Schemas/${getRequest().xsdFile}" namespace="${getRequest().xsdNs}"/>
			<xs:import schemaLocation="../Schemas/${getResponse().xsdFile}" namespace="${getResponse().xsdNs}"/>
		</xs:schema>
	</wsdl:types>

    <wsdl:message name="${getRequest().msgName}">
		<wsdl:part name="${getRequest().param}" element="req:clienteClaveCrearExpReq"/>
	</wsdl:message>
	<wsdl:message name="${getResponse().msgName}">
		<wsdl:part name="${getResponse().msgName}" element="resp:clienteClaveCrearExpResp"/>
	</wsdl:message>
