...
public class ClienteIdentidadValidarWsdlWriterTest extends FreemarkerWriterTest 
{	
	@Before
	...
	public void expTest() throws Exception
	{
		...
		OffsetParameters offsetParameters = new OffsetParameters(12, 7, 18, 34, 4, 4);		
	...
	{
		File inputHtml = new File(
				"resources/spec/UsersOOB2.html");

		InputParameters parametersIn = new InputParameters("Cliente", "UsersOOB", "loginLoginHash", "v1.0", "CORP",
				"CORP", "OSB", "v2016.08", "Vucore");
		
		OutputParameters implOutParameters = WrapParameters.implWrap(parametersIn);
		OffsetParameters offsetParameters = new OffsetParameters(2, 0, 11, 0, 0, 0);
		this.writeFiles(offsetParameters, implOutParameters, inputHtml);	
	}

	private void writeFiles(OffsetParameters offsetParameters, OutputParameters outParameters, 
			File inputHtml) 
	{	
		try {
